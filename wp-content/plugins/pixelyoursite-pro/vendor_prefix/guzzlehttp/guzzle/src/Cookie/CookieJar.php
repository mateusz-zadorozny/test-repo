...
class CookieJar implements \PYS_PRO_GLOBAL\GuzzleHttp\Cookie\CookieJarInterface
{
    ...
    public function __construct(bool $strictMode = \false, array $cookieArray = [])
    {
        ...
        foreach ($cookieArray as $cookie) {
            if (!$cookie instanceof \PYS_PRO_GLOBAL\GuzzleHttp\Cookie\SetCookie) {
                $cookie = new \PYS_PRO_GLOBAL\GuzzleHttp\Cookie\SetCookie($cookie);
            }
    ...
    public static function fromArray(array $cookies, string $domain) : self
    {
        ...
        foreach ($cookies as $name => $value) {
            $cookieJar->setCookie(new \PYS_PRO_GLOBAL\GuzzleHttp\Cookie\SetCookie(['Domain' => $domain, 'Name' => $name, 'Value' => $value, 'Discard' => \true]));
        }
        return $cookieJar;
    }
    /**
    ...
    public function extractCookies(\PYS_PRO_GLOBAL\Psr\Http\Message\RequestInterface $request, \PYS_PRO_GLOBAL\Psr\Http\Message\ResponseInterface $response) : void
    {
        if ($cookieHeader = $response->getHeader('Set-Cookie')) {
            foreach ($cookieHeader as $cookie) {
                $sc = \PYS_PRO_GLOBAL\GuzzleHttp\Cookie\SetCookie::fromString($cookie);
                if (!$sc->getDomain()) {
