...
class RedirectMiddleware
{
    ...
    public function __invoke(\PYS_PRO_GLOBAL\Psr\Http\Message\RequestInterface $request, array $options) : \PYS_PRO_GLOBAL\GuzzleHttp\Promise\PromiseInterface
    {
        ...
        return $fn($request, $options)->then(function (\PYS_PRO_GLOBAL\Psr\Http\Message\ResponseInterface $response) use($request, $options) {
    ...
    public function checkRedirect(\PYS_PRO_GLOBAL\Psr\Http\Message\RequestInterface $request, array $options, \PYS_PRO_GLOBAL\Psr\Http\Message\ResponseInterface $response)
    {
        ...
        if (\PYS_PRO_GLOBAL\GuzzleHttp\Psr7\UriComparator::isCrossOrigin($request->getUri(), $nextRequest->getUri()) && \defined('\\CURLOPT_HTTPAUTH')) {
    ...
    public function modifyRequest(\PYS_PRO_GLOBAL\Psr\Http\Message\RequestInterface $request, array $options, \PYS_PRO_GLOBAL\Psr\Http\Message\ResponseInterface $response) : \PYS_PRO_GLOBAL\Psr\Http\Message\RequestInterface
    {
        ...
        if (\PYS_PRO_GLOBAL\GuzzleHttp\Psr7\UriComparator::isCrossOrigin($request->getUri(), $modify['uri'])) {
     ...
     */
    private static function redirectUri(\PYS_PRO_GLOBAL\Psr\Http\Message\RequestInterface $request, \PYS_PRO_GLOBAL\Psr\Http\Message\ResponseInterface $response, array $protocols) : \PYS_PRO_GLOBAL\Psr\Http\Message\UriInterface
    {
        ...
        if (!\in_array($location->getScheme(), $protocols)) {
            throw new \PYS_PRO_GLOBAL\GuzzleHttp\Exception\BadResponseException(\sprintf('Redirect URI, %s, does not use one of the allowed redirect protocols: %s', $location, \implode(', ', $protocols)), $request, $response);
        }
        return $location;
    }
}
