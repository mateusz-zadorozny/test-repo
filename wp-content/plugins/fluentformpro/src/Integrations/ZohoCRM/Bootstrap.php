...
//        add_filter('fluentform_notifying_async_zohocrm', '__return_false');
    ...
    public function notify($feed, $formData, $entry, $form)
    {
        ...
        if (!$list_id) {
            do_action('ff_integration_action_result', $feed, 'failed', 'Skipped insert Zoho CRM feed data for missing Service Id.');
            return false;
        }
        ...
        foreach ($keys as $key) {
            if ($key['required'] && empty($feedData[$key['key']])) {
                do_action('ff_integration_action_result', $feed, 'failed', "Skipped insert Zoho CRM  $list_id data for missing required value. Field: " . $key['key'] . '.');
                return false;
            }
            if (! empty($feedData[$key['key']])) {
                if ('Tag' == $key['key']) {
                    $tags = $feedData[$key['key']];
                    continue;
                }
        ...
        if (is_wp_error($response)) {
            ...
            // update record tags
            if ($tags && $recordId = ArrayHelper::get($response, 'data.0.details.id')) {
                try {
