...
class InventoryValidation
{
    ...
    public function validate()
    {
        ...
        foreach ($inventoryFields as $fieldName => $item) {
            ...
            try {
                ...
                if ($e->getMessage() == 'continue') {
                    ...
                    $errors[$fieldName] = [
                        'stock-out' => wpFluentForm()->applyFilters('fluentform_inventory_validation_error',
        ...
        if (!empty($errors)) {
            ...
            $errors = $app->applyFilters('fluentform_validation_error', $errors, $this->form, $fields, $this->formData);
    ...
    private static function runSumQuantityQuery($formId)
    {
        ...
        $quantity = wpFluent()->table('fluentform_order_items')
            ->select(['fluentform_order_items.item_name','fluentform_order_items.item_price','fluentform_order_items.quantity','fluentform_order_items.parent_holder'])
            ->select(wpFluent()->raw('sum(' . $wpdb->prefix . 'fluentform_order_items.quantity) as total_count'))
